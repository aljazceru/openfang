                        ...
                        .find(|c| c.tools().iter().any(|t| t.name == other))
                    {
                        ...
                        if let Some(server_name) =
                            mcp::extract_mcp_server_from_known(other, &known_refs)
                                .or_else(|| mcp::extract_mcp_server(other))
                        {
...
fn resolve_file_path_with_policy(
    ...
    let is_full_exec =
        exec_policy.is_some_and(|p| p.mode == openfang_types::config::ExecSecurityMode::Full);
...
async fn tool_shell_exec(
    ...
    match result {
        Ok(Ok(output)) => {
            ...
            let mut stdout_str = if max_output > 0 && stdout.len() > max_output {
                format!(
                    "{}...\n[truncated, {} total bytes]",
